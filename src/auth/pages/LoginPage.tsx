  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 sm:p-6 bg-slate-50 dark:bg-[#0B1120] text-slate-900 dark:text-slate-100 font-sans transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="w-full max-w-[440px] mx-auto space-y-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-7 sm:p-9 rounded-3xl shadow-xl dark:shadow-2xl shadow-slate-900/10 relative z-10">
        ...
        {/* Quick Demo Roles Dropdown (Drill-Down Option) */}
        <div className="space-y-2">
          <div className="flex items-center justify-between px-0.5">
            <label htmlFor="demo-role-select" className="text-[11px] font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-1">
              <Sparkles size={13} className="text-amber-500" /> Select Quick Demo Role
            </label>
          ...
          <select
            ...
            onChange={(e) => {
              if (e.target.value) {
                handleDemoRoleSelect(e.target.value);
              }
            }}
            value={email}
            className="w-full rounded-xl px-3.5 py-2.5 text-xs bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-100 font-bold focus:outline-none focus:ring-2 focus:ring-blue-500/20 cursor-pointer shadow-sm transition-all"
          >
            <option value="admin@thestackly.com">Admin Portal (Full Access)</option>
            <option value="hr@thestackly.com">HR Manager Portal (Workforce Control)</option>
            <option value="manager@thestackly.com">Department Head Portal (Org Scoped)</option>
            <option value="lead@thestackly.com">Team Lead Portal (Productivity Metrics)</option>
            <option value="employee@thestackly.com">Employee Portal (Self-Service)</option>
          </select>
        </div>
